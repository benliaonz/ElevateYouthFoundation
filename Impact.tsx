...
export default function Impact() {
  ...
  return (
    <div id="impact" className="py-24 bg-indigo-50/50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-white rounded-[40px] shadow-2xl shadow-indigo-200/50 overflow-hidden border border-indigo-100">
          <div className="md:flex">
            <div className="p-10 md:p-20 md:w-1/2 flex flex-col justify-center">
              <h2 className="text-4xl font-black text-indigo-950 mb-6 leading-tight text-center md:text-left">Financial <br /><span className="text-indigo-600">Accountability</span></h2>
              ...
              <div className="space-y-6">
                {FINANCIAL_DATA.map((item) => (
                  <div key={item.name} className="flex items-center p-4 rounded-2xl hover:bg-indigo-50 transition-colors">
                    ...
                    <span className="font-black text-indigo-950 text-xl">{item.value}%</span>
                  </div>
                ))}
              </div>
            </div>

            <div className="bg-indigo-950 p-12 md:p-20 md:w-1/2 flex items-center justify-center relative overflow-hidden">
              <div className="absolute top-0 left-0 w-full h-full opacity-20 bg-[radial-gradient(circle_at_center,_var(--tw-gradient-stops))] from-indigo-400 via-transparent to-transparent"></div>
              <div className="relative w-80 h-80 z-10">
                <div 
                  className="absolute inset-0 rounded-full shadow-2xl ring-8 ring-white/5"
                  style={{ background: conicGradient }}
                ></div>
                <div className="absolute inset-10 bg-indigo-950 rounded-full flex items-center justify-center shadow-2xl border border-white/10">
                   <div className="text-center">
                     <span className="block text-4xl font-black text-white">82%</span>
                     <span className="text-[10px] text-indigo-300 font-bold uppercase tracking-[0.2em] mt-1 block">Direct Impact</span>
                   </div>
                </div>
              </div>
            </div>
